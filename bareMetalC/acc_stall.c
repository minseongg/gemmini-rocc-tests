...
int main() {
  ...
  printf("Calculate the accumulator and scratchpad addresses of all our matrices\n");
  printf("  Note: The scratchpad is \"row-addressed\", where each address contains one matrix row\n");
  size_t In_acc_addr = 0x80000000;
  size_t Out_sp_addr = DIM;
  size_t Identity_sp_addr = 2*DIM;

  printf("Move \"In\" matrix from main memory into Gemmini's accumulator\n");
  gemmini_config_ld(DIM * sizeof(elem_t));
  gemmini_config_st(DIM * sizeof(elem_t));
  gemmini_mvin(In, In_acc_addr);
  ...
  gemmini_compute_preloaded(In_acc_addr, Identity_sp_addr);
